...
class Candidate {
    ...
    //[GET] /edit_profile_candidate
    edit_profile_candidate(req, res, next) {
        try {
            if (req.isAuthenticated()) {

                const user = req.session.passport.user;

                res.render('candidate/content_edit_profile', {
                    data: {
                        user: user
                    },
    ...
    // [POST] /edit_profile_candidate
    async post_edit_profile_candidate(req, res, next) {
        try {
            if (req.isAuthenticated()) {
                ...
                const updateCandidate = await candidateModel.updateInfoCandidate(user.id, dataUpdate, req.file);

                if (updateCandidate) {
                    req.flash("message", "Cập nhật thông tin ứng thành công");
                    //res.end(); // important to update session
                } else {
                    req.flash("messageDanger", "Cập nhật thông tin ứng không thành công !");
                }
                const candidate = await candidateModel.getByEmail(user.email);
                req.session.passport.user = candidate;
                res.status(200).redirect("/candidate/edit_profile_candidate");
